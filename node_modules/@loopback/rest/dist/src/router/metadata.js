...
function api(spec) {
    return function (constructor) {
        ...
        context_1.Reflector.defineMetadata(API_SPEC_KEY, apiSpec, constructor);
    };
}
exports.api = api;
/**
 ...
 * @param spec API spec
 */
function resolveControllerSpec(constructor, spec) {
    ...
    const endpoints = context_1.Reflector.getMetadata(ENDPOINTS_KEY, constructor.prototype) || {};
    for (const op in endpoints) {
        const endpoint = endpoints[op];
        const className = endpoint.target.constructor.name ||
            constructor.name ||
            '<AnonymousClass>';
...
function getControllerSpec(constructor) {
    let spec = context_1.Reflector.getMetadata(API_SPEC_KEY, constructor);
    if (!spec) {
        spec = resolveControllerSpec(constructor, spec);
        context_1.Reflector.defineMetadata(API_SPEC_KEY, spec, constructor);
    }
    return spec;
}
...
function operation(verb, path, spec) {
    return function (target, propertyKey, descriptor) {
        assert(typeof target[propertyKey] === 'function', '@operation decorator can be applied to methods only');
        let endpoints = Object.assign({}, context_1.Reflector.getMetadata(ENDPOINTS_KEY, target));
        context_1.Reflector.defineMetadata(ENDPOINTS_KEY, endpoints, target);
        ...
        editOperationSpec(endpoint, overrides => {
            ...
            mergedSpec.responses = Object.assign({}, spec.responses, overrides.responses);
            return mergedSpec;
        });
    };
}
...
function param(paramSpec) {
    return function (target, propertyKey, descriptorOrParameterIndex) {
        assert(typeof target[propertyKey] === 'function', '@param decorator can be applied to methods only');
        let endpoints = Object.assign({}, context_1.Reflector.getMetadata(ENDPOINTS_KEY, target));
        context_1.Reflector.defineMetadata(ENDPOINTS_KEY, endpoints, target);
        let endpoint = endpoints[propertyKey];
        if (!endpoint) {
...
exports.param = param;
function editOperationSpec(endpoint, updateFn) {
...
(function (param) {
     ...
     */
    param.body = function (name, schema) {
        return param({ name, in: 'body', schema });
    };
})(param = exports.param || (exports.param = {}));
function createParamShortcut(source, type) {
